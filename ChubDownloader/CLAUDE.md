...
4. **CharacterScraper** - Core scraping logic with three modes:
   - Leaderboard downloading (followers-based)
   - Segment-based downloading (quality, trending, etc.)
   - Characters pages downloading (paginated character listing)
...
- **characters5/** - Downloaded characters from paginated listing
- **character_index.json** - Global character index to prevent duplicates

## Development Notes

The application uses Russian language for console output and supports three download modes with different character filtering criteria. The scraper implements retry logic and maintains a character index to avoid duplicate downloads.